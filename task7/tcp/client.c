...
int main() {
   ...
   int connect_value = connect(client_socket, (struct sockaddr *) &server_sockaddr_in,
   ...
   while (1) {
      ...
      ind++;
      buff[ind] = '\0';      
      int write_bytes = write(client_socket, buff, BUFF_SIZE);
      if (!strcmp(buff, "exit\n")) {
         exit(0);
      }
      int read_bytes = read(client_socket, buff, BUFF_SIZE);
